...
class _KanbanState extends State<Kanban> {
  LinkedHashMap<String, List<Item>> myMap;
  ...
  buildItemDragTarget(listId, targetPosition, double height) {
    return DragTarget<Item>(
      ...
      onWillAccept: (Item data) {
        return myMap[listId].isEmpty ||
            data.id != myMap[listId][targetPosition].id;
      },
      // Moves the card into the position
      onAccept: (Item data) {
        setState(() {
          myMap[data.listId].remove(data);
          data.listId = listId;
          if (myMap[listId].length > targetPosition) {
            myMap[listId].insert(targetPosition + 1, data);
          } else {
            myMap[listId].add(data);
          }
        });
      },
      builder:
              ...
              () {
                ...
                for (String key in myMap.keys) {
                  if (key == incomingListId) {
                    reorderedBoard[listId] = myMap[listId];
                  } else if (key == listId) {
                    reorderedBoard[incomingListId] = myMap[incomingListId];
                  } else {
                    reorderedBoard[key] = myMap[key];
                  }
                }
                myMap = reorderedBoard;
              },
            );
          },
      ...
      body: SingleChildScrollView(
        ...
        child: Row(
            ...
            children: myMap.keys.map((String key) {
              return Container(
                width: widget.tileWidth,
                child: buildKanbanList(key, myMap[key]),
              );
            }).toList()),
      ),
    );
